...
public class Main {
   ...
   static VelocityContext context;
	...
	private static final String connectedWS = "ConnectedWorkSpace.vm";
	private static final String cycleWS = "CycleWorkSpace.vm";
	private static final String finishTimeWS = "FinishTimeWorkSpace.vm";
	private static final String numberWS = "NumberWorkSpace.vm";
	private static final String transposeWS = "TransposeWorkSpace.vm";
	private static final String WS = File.separator+"WorkSpace.vm";
	...
	public static LinkedList<String> readsTemplatesFilesNames() {
		...
		if (file.isDirectory()) {
			...
			for (File templateFile: file.listFiles()) {
				...
				// if CONNECTED not selected exclude file ConnectedWorkSpace.vm
				if ( (templateFilePath.indexOf(connectedWS)!=-1)  && !Main.CONNECTED ) continue;
				
				// if CYCLE not selected exclude file CycledWorkSpace.vm
				if ( (templateFilePath.indexOf(cycleWS)!=-1)  && !Main.CYCLE) continue;
				
				// if STRONGLYCONNECTED not selected exclude file FinishTimeWorkSpace.vm
				if ( (templateFilePath.indexOf(finishTimeWS)!=-1)  && !Main.STRONGLYCONNECTED) continue;
								
				// if NUMBER not selected exclude file NumberWorkSpace.vm
				if ( (templateFilePath.indexOf(numberWS)!=-1)  && !Main.NUMBER) continue;
				
				// if STRONGLYCONNECTED not selected exclude file TransposeWorkSpace.vm
				if ( (templateFilePath.indexOf(transposeWS)!=-1)  && !Main.STRONGLYCONNECTED) continue;
				
				// WorkSpace is selected whenever BFS or DFS are selected. Thus if neither is selected and the WorkSpace.vm is found, then skip
				if ((templateFilePath.indexOf(WS)!=-1) && !Main.DFS && !Main.BFS) continue;
